...
const ProposalsListItem = ({ proposal }: ProposalsListItemProps) => {
  ...
  return (
    <div
      onClick={() => navigateToProposal(proposal.suggestSuggestionId)}
      ...
      <div className="flex gap-3.5">
        ...
        <div className="flex flex-col justify-between">
          <p className="text-button text-2xl font-bold">
            {proposal.suggestUserNickname} 전문가님의 제안
          </p>
          <p className="text-base font-bold">{proposal.suggestCenter}</p>
          <StarRating rating={proposal.suggestAverageRate} size={14} fontSize={10} />
          <p className="text-xs font-semibold">{proposal.suggestAddress}</p>
        </div>
      </div>

      <div className="mr-6 text-right text-2xl font-bold">
        <p>
          <span className="text-button">{proposal.suggestSessionCount}</span> 회
        </p>
        <p>
          <span className="text-button">{proposal.suggestPrice.toLocaleString()}</span> 원
        </p>
      </div>
    </div>
  );
