...
import ProProfileCard from '@/features/Category/components/ProProfileCard';
import ArrowRight from '@/features/home/assets/icons/ArrowRight';
import type { ProCardItem } from '@/types/ProCardItemType';

interface ProCardScrollProps {
  pros: ProCardItem[];
}

const ProCardScroll = ({ pros }: ProCardScrollProps) => {
  ...
  const navigateToProDetail = (id: number) => {
    navigate(urlFor.proDetail(id));
  };
  ...
  // 카드 실제 너비 측정해서 step 계산 (처음 한 번 + pros 변동 시)
  const measureStep = () => {
  ...
  return (
    <div className="3xl:w-[1480px] relative flex h-[400px] items-center overflow-hidden sm:w-[720px] md:w-[820px] lg:w-[920px] xl:w-[1080px] 2xl:w-[1280px]">
      ...
      >
        {pros.map(({ id, ...cardProps }) => (
          <div
            key={id}
            className="flex h-full flex-shrink-0 cursor-pointer items-center"
            onClick={() => navigateToProDetail(id)}
          >
            <ProProfileCard {...cardProps} />
          </div>
        ))}
      </div>
